...
public:   
    ...
    EnumType readEnum(const Ego::Script::EnumDescriptor<EnumType>& enumDescriptor)
    {
        ...
        {
            throw idlib::hll::compilation_error(__FILE__, __LINE__, idlib::hll::compilation_error_kind::lexical, get_location(), "invalid enum");
        }
        return it->second;
    }
    ...
    EnumType readEnum(const Ego::Script::EnumDescriptor<EnumType>& enumDescriptor, EnumType defaultValue)
    {
        ...
        if (it == enumDescriptor.end())
        {
            Log::activeTarget() << Log::Entry::create(Log::Level::Warning, __FILE__, __LINE__, "in file ", "`", get_file_name(), "`", ":", "`", name, "`",
