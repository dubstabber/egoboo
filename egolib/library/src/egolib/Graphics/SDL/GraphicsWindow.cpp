...
std::string GraphicsWindow::title() const
{
    return SDL_GetWindowTitle(window);
}

void GraphicsWindow::title(const std::string& title)
{
    SDL_SetWindowTitle(window, title.c_str());
}

void GraphicsWindow::grab_enabled(bool enabled)
{
    SDL_SetWindowGrab(window, enabled ? SDL_TRUE : SDL_FALSE);
}

bool GraphicsWindow::grab_enabled() const
{
    return SDL_TRUE == SDL_GetWindowGrab(window);
}

Vector2f GraphicsWindow::size() const
{
    int width, height;
...
void GraphicsWindow::size(const Vector2f& size)
{
    SDL_SetWindowSize(window, size.x(), size.y());
}

Point2f GraphicsWindow::position() const
{
    int x, y;
...
void GraphicsWindow::position(const Point2f& position)
{
...
Vector2f GraphicsWindow::drawable_size() const
{
    int width, height;
    ...
    while (result == 1)
    {
        switch (event.window.event)
        {
            case SDL_WINDOWEVENT_ENTER:
                this->mouse_pointer_entered(idlib::events::mouse_pointer_entered_event{});
                break;
            case SDL_WINDOWEVENT_LEAVE:
                this->mouse_pointer_exited(idlib::events::mouse_pointer_exited_event{});
                break;
            case SDL_WINDOWEVENT_FOCUS_GAINED:
                this->keyboard_input_focus_received(idlib::events::keyboard_input_focus_received_event{});
                break;
            case SDL_WINDOWEVENT_FOCUS_LOST:
                this->keyboard_input_focus_lost(idlib::events::keyboard_input_focus_lost_event{});
                break;
            case SDL_WINDOWEVENT_RESIZED:
                this->window_resized(idlib::events::window_resized_event{});
                break;
            case SDL_WINDOWEVENT_SHOWN:
                this->window_shown(idlib::events::window_shown_event{});
                break;
            case SDL_WINDOWEVENT_HIDDEN:
                this->window_hidden(idlib::events::window_hidden_event{});
                break;
...
{ 
    ...
    else
    {
        Ego::OpenGL::PushClientAttrib pca(GL_CLIENT_PIXEL_STORE_BIT);
        {
            ...
            auto drawableSize = drawable_size();
            surface =
