...
bool map_has_some_fx_itile( map_t * pmesh, int itile, uint8_t test_fx ) {
	if (!pmesh) {
		throw idlib::argument_null_error(__FILE__, __LINE__, "pmesh");
	}
...
void map_generate_fan_type_data( map_t& mesh )
{
	...
	try {
		...
		Log::activeTarget() << Log::Entry::create(Log::Level::Warning, __FILE__, __LINE__, "unable to allocate a temporary array", Log::EndOfEntry);
		throw;
	}
