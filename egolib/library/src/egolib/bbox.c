...
int oct_bb_t::to_points(const oct_bb_t& self, Ego::Vector4f pos[], size_t pos_count)
{
    /// @author BB
 ...
 */
void oct_bb_t::points_to_oct_bb(oct_bb_t& self, const Ego::Vector4f points[], const size_t numberOfPoints)
{
    if (!points)
    {
        throw idlib::argument_null_error(__FILE__, __LINE__, "points");
    }
    if (!numberOfPoints)
    {
    ...
    oct_vec_v2_t otmp(Ego::Vector3f(points[0][kX], points[0][kY], points[0][kZ]));
    ...
    {
        otmp = oct_vec_v2_t(Ego::Vector3f(points[i][kX], points[i][kY], points[i][kZ]));
