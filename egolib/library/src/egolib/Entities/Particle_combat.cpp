...
#include "egolib/game/CharacterParticleOps.h"  // chr_get_lowest_attachment
...
void Particle::updateAttachedDamage()
{
    ...
    int actual_damage = damageable.damage(ATK_BEHIND, local_damage, static_cast<DamageType>(damagetype),
                                          objectAttributionFor(owner_ref, team), getProfile()->hasBit(DAMFX_ARMO),
...
void Particle::destroy()
{
    ...
    {
        const ObjectRef childRef = moduleCommands().spawnObjectRef(getPosition(), _spawnerProfile, team, 0, facing, "", ObjectRef::Invalid);
