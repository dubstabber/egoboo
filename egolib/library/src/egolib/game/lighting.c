...
float light_a = 0.0f,
      light_d = 0.0f;
Ego::Vector3f light_nrm = idlib::zero<Ego::Vector3f>();

//--------------------------------------------------------------------------------------------

static bool lighting_sum_project( lighting_cache_t& dst, const lighting_cache_t& src, const Ego::Vector3f& vec, const int dir );

//--------------------------------------------------------------------------------------------

void lighting_vector_evaluate(const LightingVector& lvec, const Ego::Vector3f& nrm, float& dir, float& amb)
{
    dir = 0.0f;
    amb = 0.0f;
...
void lighting_vector_sum(LightingVector& lvec, const Ego::Vector3f& nrm, const float direct, const float ambient)
{
    if ( nrm.x() > 0.0f )
    {
...
void lighting_cache_t::lighting_project_cache( lighting_cache_t& dst, const lighting_cache_t& src, const Ego::Matrix4f4f& mat )
{
    ...
    Ego::Vector3f fwd = mat_getChrForward(mat);
...
//--------------------------------------------------------------------------------------------
bool lighting_sum_project( lighting_cache_t& dst, const lighting_cache_t& src, const Ego::Vector3f& vec, const int dir )
{
...
//--------------------------------------------------------------------------------------------
float lighting_cache_base_t::evaluate( const lighting_cache_base_t& self, const Ego::Vector3f& nrm, float& amb )
{
    float dir;
...
float lighting_cache_t::lighting_evaluate_cache( const lighting_cache_t& src, const Ego::Vector3f& nrm, const float z, const Ego::AxisAlignedBox3f& bbox, float * light_amb, float * light_dir )
{
...
//--------------------------------------------------------------------------------------------
bool sum_dyna_lighting( const dynalight_data_t * pdyna, LightingVector& lighting, const Ego::Vector3f& nrm )
{
...
void dynalight_data_t::init(dynalight_data_t& self)
{
	...
	self.falloff = 255.0f;
	self.level = 0.0f;
	self.pos = idlib::zero<Ego::Vector3f>();
}
