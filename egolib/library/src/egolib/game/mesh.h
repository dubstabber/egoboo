...
public:
	...
	float _edge_y;
    Ego::AxisAlignedBox3f _bbox;                 ///< bounding box for the entire mesh
...
private:
    ...
    static Ego::AxisAlignedBox2f leastClosure(const Ego::Circle2f& circle) {
        auto circle0 = Ego::Circle2f(circle.get_center(),
                                     std::max(circle.get_radius(), Info<float>::Grid::Size() * 0.5f));
        return idlib::enclose<Ego::AxisAlignedBox2f>(circle0);
    }
};
...
public:
    ...
    Ego::Vector3f get_diff(const Ego::Vector3f& pos, float radius, float center_pressure, const BIT_FIELD bits);
    float get_pressure(const Ego::Vector3f& pos, float radius, const BIT_FIELD bits) const;
    ...
    Index1D getTileIndex(const Ego::Vector2f& point) const;
	...
	BIT_FIELD hit_wall(const Ego::Vector3f& pos, float radius, const BIT_FIELD bits, Ego::Vector2f& nrm, float *pressure, const mesh_wall_data_t& data) const;
	BIT_FIELD hit_wall(const Ego::Vector3f& pos, const float radius, const BIT_FIELD bits, Ego::Vector2f& nrm, float *pressure) const;
	/// @todo @a pos and @a radius should be passed as a sphere.
	BIT_FIELD test_wall(const BIT_FIELD bits, const mesh_wall_data_t& data) const;
	BIT_FIELD test_wall(const Ego::Vector3f& pos, const float radius, const BIT_FIELD bits) const;
	 ...
	 *  0 otherwise
	 */
	float getElevation(const Ego::Vector2f& p, bool waterwalk) const;
	/**
	 * @brief
	 *  Get the precise height of the mesh at a given point (world coordinates).
	 * @param p
	 ...
	 *  0 otherwise
	 **/
	float getElevation(const Ego::Vector2f& p) const;
