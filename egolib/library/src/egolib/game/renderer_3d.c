...
bool LineSegmentList::add(const Ego::Vector3f& p, const Ego::Vector3f& q, const int duration )
{
    ...
    line.colour = Ego::Colour4f(1,1,1,0.5);
...
bool PointList::add(const Ego::Vector3f& p, const int duration)
{
    size_t index = get_free();
    if (index == capacity) return false;
    ...
    point.colour = Ego::Colour4f(1, 0, 0, 0.5);
