...
/// @file egolib/game/GameStates/SelectPlayersState.cpp
      ...
      _continueButton(std::make_shared<Ego::GUI::Button>("Select Module", SDLK_RETURN))
{
    ...
    background->setSize({ std::min(SCREEN_WIDTH, background->getTextureWidth()),
                          std::min(SCREEN_HEIGHT, background->getTextureHeight()) });
