...
SelectCharacterState::SelectCharacterState(std::shared_ptr<LoadPlayerElement> &selectedCharacter)
{
    ...
    background->setSize(Ego::Vector2f(std::min(SCREEN_WIDTH, background->getTextureWidth()), std::min(SCREEN_HEIGHT, background->getTextureHeight())));
	background->setPosition(Ego::Point2f(SCREEN_WIDTH, SCREEN_HEIGHT) + Ego::Vector2f(-background->getWidth(), -background->getHeight()));
