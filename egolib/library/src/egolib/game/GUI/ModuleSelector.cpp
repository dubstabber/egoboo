...
/// @file egolib/game/GUI/ModuleSelector.cpp
...
void ModuleSelector::drawContainer(DrawingContext& drawingContext) {
    ...
    if (_selectedModule != nullptr) {
        ...
        if (_selectedModule->getRank() > 0) {
            ...
            const std::shared_ptr<Texture> &skullTexture = EngineContext::get().textureManager().getTexture("mp_data/skull");
...
bool ModuleSelector::notifyMouseWheelTurned(const Events::MouseWheelTurnedEvent& e) {
    if (e.get_delta().y() < 0 && _startIndex == 0) {
        return false;
    }
    if (e.get_delta().y() > 0 && _startIndex >= _modules.size() - 3) {
        return false;
    }
