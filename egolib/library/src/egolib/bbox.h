...
/// The various axes for the octagonal bounding box
    ...
    public:
        ...
        void add(const Ego::Vector3f& other)
        {
        ...
        void sub(const Ego::Vector3f& other)
        {
    ...
    public:
		...
		oct_vec_v2_t(const Ego::Vector3f& point)
...
//--------------------------------------------------------------------------------------------
	...
	public:
         ...
         */
		Ego::AxisAlignedBox3f toAxisAlignedBox() const {
            if (_empty) {
                throw std::logic_error("unable to convert an empty OBB into an AABB");
            }
            return Ego::AxisAlignedBox3f(Ego::Point3f(_mins[OCT_X], _mins[OCT_Y], _mins[OCT_Z]),
				                         Ego::Point3f(_maxs[OCT_X], _maxs[OCT_Y], _maxs[OCT_Z]));
        }
	...
	public:
		...
		static int to_points(const oct_bb_t& self, Ego::Vector4f pos[], size_t pos_count);
		static void points_to_oct_bb(oct_bb_t& self, const Ego::Vector4f pos[], const size_t pos_count);
...
template <>
struct translate_functor<oct_bb_t, Ego::Vector3f>
{
	oct_bb_t operator()(const oct_bb_t& x, const Ego::Vector3f& t) const
