...
bool map_info_t::validate() const
{
    ...
    {
        Log::activeTarget() << Log::Entry::create(Log::Level::Warning, __FILE__, __LINE__, "too many vertices - received ",
    ...
    {
        Log::activeTarget() << Log::Entry::create(Log::Level::Warning, __FILE__, __LINE__, "too many files in the x direction",
    ...
    {
        Log::activeTarget() << Log::Entry::create(Log::Level::Warning, __FILE__, __LINE__, "too many tiles in the y direction ",
    ...
    {
		Log::activeTarget() << Log::Entry::create(Log::Level::Warning, __FILE__, __LINE__, "mesh is too large - received ", tileCount, " vertices, expected not more than ", MAP_TILE_MAX, " vertices", Log::EndOfEntry);
        return false;
    }
...
bool map_t::load(vfs_FILE& file)
{
    ...
    try
    {
        if (mapVersion <= 0)
        {
            Log::activeTarget() << Log::Entry::create(Log::Level::Warning, __FILE__, __LINE__, "unknown map type!", Log::EndOfEntry);
            goto Fail;
        }
        else if (mapVersion > CURRENT_MAP_VERSION_NUMBER)
        {
			Log::activeTarget() << Log::Entry::create(Log::Level::Warning, __FILE__, __LINE__, "file version ", mapVersion, " is "
        ...
        if (!setInfo(loc_info))
        {
			Log::activeTarget() << Log::Entry::create(Log::Level::Warning, __FILE__, __LINE__, "unable to initialize map", Log::EndOfEntry);
            goto Fail;
        }
...
bool map_t::load(const std::string& name)
{
    ...
    if (!file)
    {
		Log::activeTarget() << Log::Entry::create(Log::Level::Warning, __FILE__, __LINE__, "unable to find map file", "`", name, "`", Log::EndOfEntry);
        goto Fail;
    }
