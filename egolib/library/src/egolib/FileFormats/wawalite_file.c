...
static Ego::Vector2f readVector2f(ReadContext& ctxt);
static Ego::Vector3f readVector3f(ReadContext& ctxt);
...
void wawalite_water_t::read(ReadContext& ctxt, wawalite_water_t *profile)
{
    if (!profile)
    {
        throw idlib::argument_null_error(__FILE__, __LINE__, "profile");
    }
