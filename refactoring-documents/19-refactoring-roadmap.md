...
**Strategic note (2026-06-11 scout-next-heavy-front workflow, 8 fronts adversarially verified):** the safe **move-only modularization vein is exhausted**. A 6th link-split out of `egolib-library` (Front B) is **refuted as a direct move** — every candidate cluster is circular today (measured by nm on the live archive: ScriptVM **91** back-edges, game-graphics-render **74**, GameStates **63**, the latter also topologically at the *top* of the call graph). Carving any of them needs flag-day-scale `EngineContext`/`GameSessionContext` service-interface seam-cutting first (same class as the deferred Entities ownership-inversion). Other fronts: mechanical C++ modernization is **do-not-attempt** (no `-Wold-style-cast`/clang-tidy ratchet, so cast regressions are undetectable — premise also inflated 2–6×); portability T2.2/T2.5 are un-gateable in the headless sandbox (no display/Wine/mingw cross). **Chosen program — "de-risk → carve GameStates":** (step 1, DONE) lay a GUI characterization net (`GuiComponentBehavior.cpp`, 44 tests); (step 2, DONE) free the `GameState` base of its only game-core coupling via the `activeGameEngine()` ownership-move seam (`GameState.cpp.o` now `EngineContext`-free, nm-proven; seam stays in egolib-library, topology-neutral — the `egolib-gamestates` carve itself is **deferred** because only 4 leaf screens become nm-clean and GameStates is topologically the wrong shape for a below-remainder layer); (step 3, next) enlarge the clean cohort by seaming the LIGHT 2–4-`EngineContext` screens (`AudioOptionsScreen`, `MainMenuState`, `OptionsScreen`, `Select{Character,Module,Players}State`, `VideoOptionsScreen`, `MapEditorSelectModuleState`) onto the existing `active*()` seams, then reassess whether the cohort is large enough to be worth a layer.
