...
**Strategic note (2026-06-11 scout-next-heavy-front workflow, 8 fronts adversarially verified):** the safe **move-only modularization vein is exhausted**. A 6th link-split out of `egolib-library` (Front B) is **refuted as a direct move** — every candidate cluster is circular today (measured by nm on the live archive: ScriptVM **91** back-edges, game-graphics-render **74**, GameStates **63**, the latter also topologically at the *top* of the call graph). Carving any of them needs flag-day-scale `EngineContext`/`GameSessionContext` service-interface seam-cutting first (same class as the deferred Entities ownership-inversion). Other fronts: mechanical C++ modernization is **do-not-attempt** (no `-Wold-style-cast`/clang-tidy ratchet, so cast regressions are undetectable — premise also inflated 2–6×); portability T2.2/T2.5 are un-gateable in the headless sandbox (no display/Wine/mingw cross). **Chosen program — "de-risk → carve GameStates":** (step 1, DONE) lay a GUI characterization net (`GuiComponentBehavior.cpp`); (step 2, next) relocate the `GameState` base class (deps: `Container.hpp` [already in egolib-gui] + `EngineContext.hpp` only) to break menu-screen vtable/typeinfo back-edges, then measure the most-isolated leaf screen (`DebugFontRenderingState`) to prove it reaches the remainder only through existing `active*()`/`EngineContext` service seams — the first bounded seam-cut toward an eventual `egolib-gamestates` archive.
