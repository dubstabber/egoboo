...
**Discovered, deferred:** one pre-existing back-edge remains — `game/mesh_geometry.c` (base) reads `g_meshLookupTables` (defined in `egolib-physics`), introduced by the earlier mesh_geometry absorption. `mesh_geometry.c` can't move up (other base TUs depend on its `ego_mesh_t`/`tile_mem_t` methods); `MeshLookupTables` can't trivially move down (its table init reads the physics `g_environment` for `twist_vel`). Needs a small data-split pass.
