...
**Residual / next step.** `Collidable.hpp` is game-include-free, but `Collidable` still physically lives in `game/Physics/` and `Collidable.cpp`'s `setPosition`/`setSpawnPosition` call `activeModule()` (GameModule) for the bounds-check + tile lookup. **Physically relocating `Collidable` to a lower-layer (`egolib/`) directory — into a future `egolib-physics` sub-library — is blocked on decoupling that `activeModule()` position-validation dependency** (inject the mesh / relocate the validation). Relocating the header alone while the impl still reaches up would be a false lower-layer signal (the impl would not link without `game/`), so it is deliberately *not* done here. That ownership change is the behavior-sensitive next front, now protected by the `CombatDamageIntegration.cpp` + `ObjectAccessors` position/respawn fixtures.
