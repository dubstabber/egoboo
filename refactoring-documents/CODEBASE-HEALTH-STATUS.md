...
| Test files / lines | 5 / 383 | 56 / 27,146 |
| Test cases | 10 | 1,001 |
...
the test suite is 71× larger), but every structural-debt metric collapsed. The
three mutable globals that defined the old architecture are gone from active
code, the 8,000-line monoliths are gone, and behavior is now pinned by 1,001
...
| Test files / lines | 56 / 27,146 | `egolib/tests`, source/header files only |
| ctest cases | 1,001 | `ctest --test-dir build -N` |
| ctest baseline | 1,001 / 1,001 | Last recorded green baseline in the pass log; use `ctest -j20 --output-on-failure` |
