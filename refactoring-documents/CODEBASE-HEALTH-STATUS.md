...
- The test suite is substantially larger than the April baseline and currently
  configures 1,056 ctest cases. Every one of the 404 `scr_*` script dispatch
...
| Test files / lines | 5 / 383 | 59 / 29,497 |
| Test cases | 10 | 1,056 |
...
the test suite is 77× larger), but every structural-debt metric collapsed. The
three mutable globals that defined the old architecture are gone from active
code, the 8,000-line monoliths are gone, and behavior is now pinned by 1,056
...
| Test files / lines | 59 / 29,497 | `egolib/tests`, source/header files only |
| ctest cases | 1,056 | `ctest --test-dir build -N` |
| ctest baseline | 1,056 / 1,056 | Last recorded green baseline in the pass log; use `ctest -j20 --output-on-failure` |
