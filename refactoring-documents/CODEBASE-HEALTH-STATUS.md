...
Verified against the live tree on 2026-06-11. These are the single source of truth; sections below and sibling docs (`01`, `02`, `AGENTS.md`) reference this table rather than re-stating the figures.

| Metric | Value | Note |
| ------ | ----: | ---- |
| Active source files (egolib+egoboo, excl. tests) | **684** | `.c` 75 · `.cpp` 243 · `.h` 62 · `.hpp` 304 (incl. the lower-layer `egolib/Mesh/ITerrainQuery.hpp`) |
| Active source lines (egolib+egoboo) | ~122,600 | — |
| Test lines / ratio | ~22,000 / **~17.7%** | 44 test `.cpp` files, **875** ctest cases (incl. `GuiComponentBehavior.cpp` (44), `AITerrainQueries.cpp`, `CombatDamageIntegration.cpp`, `CollisionPipeline.cpp`) |
| ctest result | **875 / 875** | clean on this machine; the two historical `ScriptLoaderFixture` PrimaryScript-fallback cases now pass here |
...
| `Object.hpp` | **1,613** lines | monolithic by interface |
| `vfs.c` | **1,500** lines | was 2,456 before T3.6; 1,920 before the 06-11 RWops/mount split |
...
| `egolib-foundation-base`     | Static library       | Dependency-closed lowest layer (145 TUs): Math, Log, Mesh, VFS, Time, Core/System bootstrap, FileFormats, Platform, InputControl, Image, AI path/LOS terrain queries, QuestLog, Profiles data/writers, MD2 + texture/font clusters (incl. the `activeTextureManager` seam), Script DDL/PDL lexer, Logic/Perk/TreasureTables, toplevel math/IO — verified acyclic | ~31,200 |
...
| `egolib-gui`                 | Static library       | Generic GUI widget toolkit (22 TUs): Component/Container/Layout/widgets + UIManager + DrawingContext/Material — a cohesive middle layer above `egolib-renderer`; game-state-free (reaches services via `active*()` seams). Game-coupled widgets stay in `egolib-library`. | ~6,000 |
| `egolib-library`             | Static library       | Game-core remainder (98 TUs): Entities/game core, the EgoScript VM, the `GameState` base, and remaining higher gameplay/UI code — depends one-way on `egolib-physics`, `egolib-renderer`, and `egolib-gui` | ~66,000  |
| `egolib-gamestates`          | Static library       | The concrete `GameState` screens (19 TUs): menus, options, select, loading, playing, map-editor, debug, victory — the one layer **above** `egolib-library` (screens orchestrate the game, reaching down into game-core). Reaches the active in-game state via the `IPlayingStateController` seam; the initial `MainMenuState` is injected from `egoboo/Main.cpp`. | ~6,000 |
...
The historical `GLOB_RECURSE` in `egolib/library/CMakeLists.txt` has been replaced with explicit, per-subsystem source lists (one `set()` block per directory, grouped alphabetically). Ownership is visible in the build system, and as of 2026-06-11 the build enforces an acyclic **six-archive** DAG: `egolib-foundation-base` (145 TUs) ← sibling middle layers `egolib-physics` (5 TUs) and `egolib-renderer` (28 TUs) ← `egolib-gui` (22 TUs, above renderer) ← `egolib-library` (98 TUs) ← `egolib-gamestates` (19 TUs, the concrete screens, the **first ABOVE-library layer**). The dependency direction is nm symbol-closure verified with live positive controls, and the DAG is **fully acyclic — zero known back-edges** (`egolib-library → egolib-gamestates` = 0; all four lower archives → gamestates = 0). The gamestates carve is the first *upward* split — it required seam-cutting the `library → screen` reverse edges (`IPlayingStateController` interface via dynamic_cast-to-interface, a `GameEngine` main-menu-state factory, the `endModuleInVictory` seam), not just a source-list move.
...
- **`egolib` is now six static archives, not one** — `egolib-foundation-base` (145 TUs) with sibling middle layers `egolib-physics` (5 TUs) and `egolib-renderer` (28 TUs), then `egolib-gui` (22 TUs) above renderer, feeding `egolib-library` (98 TUs), with `egolib-gamestates` (19 TUs, the concrete screens) the first layer **above** library — verified fully acyclic. The remaining *higher game-core* subsystems (Entities, the EgoScript VM, the rest of game/UI) still share `egolib-library` and do not yet enforce dependency direction among themselves at link time — further sub-libraries (e.g. an upward ScriptVM or render carve) remain ahead.
...
| Build system               | 3.5/5   |   ↗   | Explicit source lists, validator integrated, six-archive DAG (`egolib-foundation-base` 145 ← `{egolib-physics 5, egolib-renderer 28}` ← `egolib-gui 22` ← `egolib-library` 98 ← `egolib-gamestates` 19, acyclic) |
...
| Module boundaries          | 2.5/5   |   ↗   | Six-archive DAG landed (`egolib-foundation-base` 145 ← `{egolib-physics 5, egolib-renderer 28}` ← `egolib-gui 22` ← `egolib-library` 98 ← `egolib-gamestates` 19, acyclic; gamestates is the first ABOVE-library layer); higher game-core subsystems still share `egolib-library` |
...
6. **Build system makes structure visible — and now enforces a six-archive DAG.** Explicit per-subsystem source lists plus the carved `egolib-foundation-base` ← `{egolib-physics, egolib-renderer}` ← `egolib-gui` ← `egolib-library` ← `egolib-gamestates` DAG (dependency-closed and verified fully acyclic, including the first ABOVE-library layer) mean dependency direction is enforced at link time; further sub-libraries are still ahead.
...
9. **`egolib` is now six static archives (was one).** The dependency-closed lower layers are real link targets (`egolib-foundation-base` 145 with `egolib-physics` 5, `egolib-renderer` 28, `egolib-gui` 22), feeding `egolib-library` 98, with `egolib-gamestates` 19 the first layer above it; the remaining higher game-core subsystems still share `egolib-library`, so much of that decomposition is still expressed only in directories and source-list blocks.
