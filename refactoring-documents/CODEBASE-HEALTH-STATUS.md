...
eliminated (409 `::get()` sites), `Object` is still the broadest interface in
...
| `::get()` call sites | 409 | `rg "::get\\(" egolib/library/src`; includes intentional context seams |
| `EngineContext::get()` | 330 | Dominant intentional engine seam; Passes 313-315 threaded the renderer, `IGFX`, and the HUD debug services through the render/HUD chains; Passes 316-317 consolidated the `graphic.c` fetch clusters and threaded `IParticleHandler` through the particle render chain |
