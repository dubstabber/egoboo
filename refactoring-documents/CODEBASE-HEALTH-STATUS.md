...
| Runtime source lines | 128,786 | Same scope as above |
| Test files / lines | 50 / 24,567 | `egolib/tests`, source/header files only |
| ctest cases | 933 | `ctest --test-dir build -N` |
| ctest baseline | 933 / 933 | Last recorded green baseline in the pass log; use `ctest -j20 --output-on-failure` |
